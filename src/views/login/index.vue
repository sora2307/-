...
const validatePass= (rule, value, callback) => {
    ...
    if(value === ''){
        ...
        reg.test(value) ? callback() : callback(new Error('密码格式不对,需要4到16位字母/数字/下划线/减号，请确认格式'))
    }
}
            ...
            if (formType.value){
                ...
                login(loginForm).then(({data}) =>{
                    if(data.code === 10000){
                        ...
                        console.log(data)
                        //将token和用户信息缓存到浏览器
                        localStorage.setItem('wx_token', data.data.token)
                        localStorage.setItem('wx_userInfo', JSON.stringify(data.data.userInfo))
                        router.push('/')
                    }
                })
            }
